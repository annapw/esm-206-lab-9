...
#Useful for interpreting multiple variables. 
...
homes_lm1 <- lm(formula = Price ~ City + Bedrooms+ Bathrooms + PricePerSqFt + Status, data = homes_sub)
summary(homes_lm1) #summary() shows the model with summary statistics

#price per sq ft is calculated from total price of each home...so there is already a direct relationship; if you include this variable, it will improve the model fit (perhaps not in a good way) because it's skewed. 

homes_lm2 <- lm(formula = Price ~ City + SqFt + Status, data = homes_sub)

# b. Model summary:
summary(homes_lm2)

#multiple r sq: one predictor, one outcome. model fit will not go up with more variables
#adjusted r sq: model fit will go up with multiple variables. 
```
...
# Use Levels(homes_sub$status)

# c. Check levels:
#fct_relevel() function in forcasts
...
#only valuable when you are comparing multiple models

# Answer: which would you pick? 
# while the first looks preferable, it does not accurately represent the trends in real life. 
```
...
stargazer(homes_lm1, homes_lm2, homes_lm3, type = "html")
...
predict_graph <- ggplot(predict_df, aes(x= SqFt, y = fit.fit))+
  ...
  geom_point(data = homes_sub, aes(x= SqFt, y= Price), alpha= 0.1)+ #adds data from a different dataframe
  facet_wrap(~Status)
